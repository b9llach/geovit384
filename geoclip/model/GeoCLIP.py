...
from ..train.dataloader import GeoDataLoader, img_val_transform

class GeoCLIP(nn.Module):
    def __init__(self, from_pretrained=True, queue_size=4096, gps_gallery_file=None):
        super().__init__()
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.image_encoder = ImageEncoder()
        self.location_encoder = LocationEncoder()

        if gps_gallery_file is None:
            gps_gallery_file = os.path.join(file_dir, "/data/geolocation/", "panorama_log_new.csv")
        
        # Use GeoDataLoader to load GPS gallery data
        gps_gallery_loader = GeoDataLoader(gps_gallery_file, "", transform=None)
        self.gps_gallery = torch.tensor(gps_gallery_loader.gps_data, dtype=torch.float32)
    ...
    def dequeue_and_enqueue(self, gps):
        ...
        # Handle case where batch size exceeds remaining space in queue
        if gps_ptr + gps_batch_size > self.queue_size:
            # Fill the rest of the queue
            self.gps_queue[:, gps_ptr:] = gps[:self.queue_size - gps_ptr].t()
            # Wrap around to the beginning of the queue
            remaining = gps_batch_size - (self.queue_size - gps_ptr)
            self.gps_queue[:, :remaining] = gps[self.queue_size - gps_ptr:].t()
            gps_ptr = remaining
        else:
            # Normal case: add batch to queue
